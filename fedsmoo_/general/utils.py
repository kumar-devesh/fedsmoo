...
max_norm = 10
# --- Evaluate a NN model
def get_acc_loss(device, data_x, data_y, model, dataset_name, w_decay = None):
...
def set_client_from_params(device, mdl, params):
...
def set_H_param_list(model_list, m, device):
    H_element_list = []
    idx = 0
    for name, param in model_list.named_parameters():
